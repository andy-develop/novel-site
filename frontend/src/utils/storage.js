/**
 * LocalStorage utility for novel-site
 * Three independent storage keys:
 *   readHistory  - reading history
 *   userBookshelf - bookshelf books
 *   bookshelfTipIgnore - books permanently declined bookshelf popup
 */
...
export function saveReadHistory(entry) {
  // entry: { bookId, bookTitle, cover, chapterId, chapterTitle, timestamp }
...
export function addToBookshelf(book) {
  // book: { bookId, title, cover, addedAt }
...
// ---- bookshelfTipIgnore ----
export function getTipIgnore() {
  return load(KEYS.tipIgnore)
}

export function isTipIgnored(bookId) {
  return load(KEYS.tipIgnore).includes(bookId)
}
